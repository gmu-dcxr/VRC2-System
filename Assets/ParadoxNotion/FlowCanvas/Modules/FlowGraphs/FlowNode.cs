...
namespace FlowCanvas
{
    ...
    abstract public partial class FlowNode : Node, ISerializationCallbackReceiver
    {
        ...
        {
            public GatherPortsCallbackAttribute() : base(nameof(GatherPorts)) { }
        }

        ///----------------------------------------------------------------------------------------------

        [SerializeField] //the user defined input port values used for serialization
        ...
        void ISerializationCallbackReceiver.OnBeforeSerialize() {
            ...
            //halt serialization for refelected nodes with missing reference
            if ( this.Is<IReflectedWrapper>(out var wrapper) ) {
                if ( wrapper.GetSerializedInfo()?.AsMemberInfo() == null ) {
                    return;
                }
            }
        ...
        static void DrawInputPortGraphic(Port port, Port contextPort, bool isSelfInstancePort) {
            ...
            if ( !port.isConnected && port is ValueInput ) {
                ...
                if ( !( port as ValueInput ).isDefaultValue ) {
                    ...
                    labelString = string.Format("<color=#{0}><b>Self</b></color>", color);
                } else {
