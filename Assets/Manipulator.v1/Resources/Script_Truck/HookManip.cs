...
public class HookManip : MonoBehaviour {
	...
	[HideInInspector]
	public CargoManipulator m_Cargo;
	[HideInInspector]
	public Manipulator m_ScriptHook_1;
	[HideInInspector]
	public ControllerTruck m_ScriptHook_2;
	private bool hook_Bool = true;
	private float limitHook = 0f;
	private bool onCol = true;
	[HideInInspector]
	public bool onCol_Cargo = true;
	private Transform lineRen1A;
	private Transform lineRen2A;
	private Transform lineRen1B;
	private Transform lineRen2B;
	[HideInInspector]
	public Transform lineArrow1For;
	[HideInInspector]
	public Transform lineArrow2For;
	[HideInInspector]
	public Transform decalHook;
	private float distanceHook_float = 0;
	[HideInInspector]
	public float distanceHook_Int = 0;
	[HideInInspector]
	...
	void Start(){
		...
		if (m_ScriptHook_1.m_ScriptM.startTruck == true) {
			if (m_ScriptHook_1.checkManip == 1 || m_ScriptHook_1.checkManip == 2) {
				lineRen1A =	m_ScriptHook_1.transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (0);
				lineRen2A =	m_ScriptHook_1.transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (1);
				...
				lineRen1A =	m_ScriptHook_1.transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (0);
				lineRen2A =	m_ScriptHook_1.transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (0).transform.GetChild (1);
				lineRen1B = transform.GetChild (0);
				lineRen2B = transform.GetChild (1);
		...
		if (m_ScriptHook_2.fps_Bool == false) {
			...
			if (hook_Bool == false) {
				if (m_ScriptHook_1.uiManip_Bool == false) {
					//Check Distance Hook
					distanceHook_float = Vector3.Distance (m_ScriptHook_1.checkDistanceHook_A.position, m_ScriptHook_1.checkDistanceHook_B.position);
	...
	public void MoveLine(){
		if (m_ScriptHook_1.checkManip == 1 || m_ScriptHook_1.checkManip == 2) {
			if (Input.GetKey (m_ScriptHook_2.DownArrow)) {
				lineRen2A.transform.localPosition = Vector3.MoveTowards (lineRen2A.transform.localPosition, new Vector3 (-0.006336521f, -0.1862f, -3.9628f), 0.01f * Time.deltaTime);
			}else	if (Input.GetKey (m_ScriptHook_2.UpArrow)) {
				lineRen2A.transform.localPosition = Vector3.MoveTowards (lineRen2A.transform.localPosition, new Vector3 (-0.006336521f, -0.237f, -3.9628f), 0.01f * Time.deltaTime);
			}
			} else if (m_ScriptHook_1.checkManip == 3) {
			if (Input.GetKey (m_ScriptHook_2.DownArrow)) {
				lineRen2A.transform.localPosition = Vector3.MoveTowards (lineRen2A.transform.localPosition, new Vector3 (0.01227948f, -0.1604001f, -0.1042f), 0.01f * Time.deltaTime);
			}else	if (Input.GetKey (m_ScriptHook_2.UpArrow)) {
				lineRen2A.transform.localPosition = Vector3.MoveTowards (lineRen2A.transform.localPosition, new Vector3 (0.01227948f, -0.207f, -0.1042f), 0.01f * Time.deltaTime);
			}
		}
	}
}
